...
func TestProcessInteractiveEvents_StaleResultGraceWindow_FollowUpArrives(t *testing.T) {
	...
	go func() {
		e.processInteractiveEvents(state, session, e.sessions, sessionKey, "m-stale-grace-followup", time.Now(), nil, nil, state.replyCtx)
		close(done)
	}()
	select {
	case <-done:
...
func TestProcessInteractiveEvents_StaleResultGraceWindow_Expires(t *testing.T) {
	...
	go func() {
		e.processInteractiveEvents(state, session, e.sessions, sessionKey, "m-stale-grace-expire", time.Now(), nil, nil, state.replyCtx)
		close(done)
	}()
	select {
	case <-done:
...
func TestProcessInteractiveEvents_StaleResultGraceWindow_NonEmptyResultSkipsGrace(t *testing.T) {
	...
	go func() {
		e.processInteractiveEvents(state, session, e.sessions, sessionKey, "m-stale-grace-skipped", time.Now(), nil, nil, state.replyCtx)
		close(done)
	}()
	select {
	case <-done:
...
func TestProcessInteractiveEvents_StaleResultGraceWindow_DeadAgentSkipsGrace(t *testing.T) {
	...
	go func() {
		e.processInteractiveEvents(state, session, e.sessions, sessionKey, "m-stale-grace-deadagent", time.Now(), nil, nil, state.replyCtx)
		close(done)
	}()
	select {
	case <-done:
...
func TestProcessInteractiveEvents_StaleResultGraceWindow_NonDoneResultSkipsGrace(t *testing.T) {
	...
	go func() {
		e.processInteractiveEvents(state, session, e.sessions, sessionKey, "m-stale-grace-nondone", time.Now(), nil, nil, state.replyCtx)
		close(doneCh)
	}()
	select {
	case <-doneCh:
...
func TestProcessInteractiveEvents_StaleResultGraceWindow_DisabledByZeroValue(t *testing.T) {
	...
	doneCh := make(chan struct{})
	go func() {
		e.processInteractiveEvents(state, session, e.sessions, sessionKey, "m-stale-grace-disabled", time.Now(), nil, nil, state.replyCtx)
		close(doneCh)
	}()
	select {
	case <-doneCh:
